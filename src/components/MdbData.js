...
class MdbData extends Component {
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (JSON.stringify(this.props) !== JSON.stringify(nextProps) && nextProps.content_type && nextProps.language && nextProps.upload_type ) {
            // Yeah it's look long :)
            let path = nextProps.content_type.match(/^(FULL_LESSON|VIRTUAL_LESSON)$/) ? `?&page_size=1000&content_type=${nextProps.content_type}&published=true` : `?&page_size=1000&content_type=${nextProps.content_type}&start_date=${nextProps.start_date}&end_date=${nextProps.end_date}`
